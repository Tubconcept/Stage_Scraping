"""
Moteur CSS du scraper Prolians (mode : tracking).
...
L'orchestration (run, boucle principale, persistance) se trouve dans scrap_prolians_tracking.py.
"""
import sys
...
def collect_orders_with_tracking(page, date_inf, date_sup):
    """Collecte les commandes + URL 'Suivre ma commande' depuis la liste."""
    orders = []
    seen   = set()
...
def get_order_detail(page, order: dict) -> dict | None:
    ...
    if tracking_url:
        ...
        # Pas de page de suivi → tenter d'extraire depuis la page de détail déjà chargée
        try:
            page.goto(detail_url, wait_until="domcontentloaded", timeout=15000)
