    ...
    def _make_panel(self, key: str) -> tk.Frame:
        ...
        if key == "produits":
            ...
            tk.Label(frm.input_area,
                     text="Catalogue ENTIER via le sitemap (~78 900 fiches DOM complètes). "
                          "Job long (plusieurs jours) mais REPRISE intégrée : arrêt/relance "
                          "reprend où il en était (saute les fiches déjà en base).",
                     font=("Helvetica", 9, "italic"), bg=BG, fg=GRAY,
                     wraplength=620, justify="center").pack(pady=(2, 4))
            frm.headless_var = tk.BooleanVar(value=True)
            tk.Checkbutton(
                frm.input_area,
                text="Mode headless (sans fenêtre — run de fond non surveillé)",
                variable=frm.headless_var,
